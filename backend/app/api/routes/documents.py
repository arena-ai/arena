...
async def create_file(
    *, current_user: CurrentUser, upload: UploadFile
) -> Document:
    name: str = str(uuid())
    document = Document(
        name=name,
    ...
    documents.puts(
        f"{current_user.id}/{name}/metadata", document.model_dump_json()
    )
    documents.puts(
        f"{current_user.id}/{name}/content_type", upload.content_type
    )
    return document
...
async def read_files(*, current_user: CurrentUser) -> Documents:
    ...
    return Documents(
        data=sorted(
            [
                Document.model_validate_json(documents.gets(f"{path}metadata"))
