from typing import Mapping, TypeVar, Generic, Mapping
...
class OpenAIHandler(ChatCompletionHandler[openai_models.ChatCompletionRequest, openai_models.ChatCompletionResponse]):
    def validate_chat_completion_request(self, chat_completion_request: Mapping) -> openai_models.ChatCompletionRequest:
        return openai_models.ChatCompletionRequest.model_validate(chat_completion_request)
    ...
    def lm_request(self) -> Computation[Request[openai_models.ChatCompletionRequest]]:
        return openai_request(self.chat_completion_request.model_copy(deep=True))

    def lm_response(self, ses: Computation[Session], usr: Computation[UserOut], request: Request[openai_models.ChatCompletionRequest]) -> Computation[Response[openai_models.ChatCompletionResponse]]:
        return openai(openai_api_key(ses, usr), request.content)


class MistralHandler(ChatCompletionHandler[mistral_models.ChatCompletionRequest, mistral_models.ChatCompletionResponse]):
    def validate_chat_completion_request(self, chat_completion_request: Mapping) -> mistral_models.ChatCompletionRequest:
        return mistral_models.ChatCompletionRequest.model_validate(chat_completion_request)
    ...
    def lm_request(self) -> Computation[Request[mistral_models.ChatCompletionRequest]]:
        return mistral_request(self.chat_completion_request.model_copy(deep=True))

    def lm_response(self, ses: Computation[Session], usr: Computation[UserOut], request: Request[mistral_models.ChatCompletionRequest]) -> Computation[Response[mistral_models.ChatCompletionResponse]]:
        return mistral(mistral_api_key(ses, usr), request.content)


class AnthropicHandler(ChatCompletionHandler[anthropic_models.ChatCompletionRequest, anthropic_models.ChatCompletionResponse]):
    def validate_chat_completion_request(self, chat_completion_request: Mapping) -> anthropic_models.ChatCompletionRequest:
        return anthropic_models.ChatCompletionRequest.model_validate(chat_completion_request)
    ...
    def lm_request(self) -> Computation[Request[anthropic_models.ChatCompletionRequest]]:
        return anthropic_request(self.chat_completion_request.model_copy(deep=True))

    def lm_response(self, ses: Computation[Session], usr: Computation[UserOut], request: Request[anthropic_models.ChatCompletionRequest]) -> Computation[Response[anthropic_models.ChatCompletionResponse]]:
