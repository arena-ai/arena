...
class LogEvent(Op[tuple[Session, User, EventOut | None, A], EventOut], Generic[A]):
    name: str
    ...
    async def call(self, session: Session, user: User, parent: EventOut | None, a: A) -> EventOut:
...
class CreateEventIdentifier(Op[tuple[Session, User, EventOut, str], EventIdentifier]):
    async def call(self, session: Session, user: User, event: EventOut, identifier: str) -> EventIdentifier:
