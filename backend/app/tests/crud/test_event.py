...
def test_delete_event(db: Session) -> None:
    ...
    bob = crud.create_user(session=db, user_create=UserCreate(email=random_email(), password=random_lower_string()))
    parent_event = crud.create_event(session=db, event_in=EventCreate(name="parent", content=random_lower_string()), owner_id=alice.id)
    children = [crud.create_event(session=db, event_in=EventCreate(name="test_request", content=random_lower_string(), parent_id=parent_event.id), owner_id=alice.id) for i in range(10)]
