...
def test_documents(object_store: Minio) -> None:
    ...
    for i in range(10):
        f = io.BytesIO(
            b"some initial binary data: \x00\x01 " + f"{i}".encode()
        )
        docs.put(f"obj_{i}", f)
    ...
    docs.puts("user/test_string", "Hello world")
    print(docs.gets("user/test_string"))
    print(
        f"After removal we have the following objects {docs.list() + docs.list(prefix='user/')}"
    )
    print(f'user/test_string exists: {docs.exists("user/test_string")}')
    print(f'user/unknown exists: {docs.exists("user/unknown")}')
